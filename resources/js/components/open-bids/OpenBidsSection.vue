...
function buttonLabel(bid: Bid): string {
    if (progressPct(bid) >= 100) return 'Last Chance Bid';
...
function getRemainingTime(endsAt: string | null | undefined): string {
    ...
    if (diffInMs <= 0) return '0 hour(s), 0 minute(s)';
...
<template>
    <section class="mx-auto max-w-screen-2xl px-6 pt-12 pb-24">
            ...
            <div class="flex flex-col justify-between gap-8 md:flex-row md:items-end">
                ...
                <div class="flex w-full flex-col gap-4 sm:flex-row md:w-auto">
                    ...
                    <div class="group relative">
                        <span class="material-symbols-outlined absolute top-1/2 left-4 -translate-y-1/2 text-outline">search</span>
                        <input
                            v-model="search"
                            type="search"
            ...
            >
                ...
                <div class="relative aspect-4/3 overflow-hidden">
                    ...
                    <div class="absolute bottom-4 left-4">
                        <span class="rounded-lg bg-primary-container px-3 py-1 text-[10px] font-black tracking-widest text-on-primary-container uppercase shadow-lg">
                            Live
                        </span>
                    </div>
                </div>
                ...
                <div class="flex grow flex-col p-6">
                    <div class="mb-4 flex items-start justify-between">
                        <h3 class="font-headline text-xl leading-tight font-extrabold">{{ bid.name }}</h3>
                        <div class="text-right">
                            <p class="mb-1 text-[10px] font-bold tracking-wider text-secondary uppercase">Price</p>
                            <p class="text-xl font-black text-primary">{{ formattedPrice(bid.price) }}</p>
                        </div>
                    </div>
                    ...
                    <div class="mt-auto">
                        <div class="mb-2 flex items-center justify-between">
                            <span class="text-xs font-bold text-secondary">
                                Progress: {{ bid.bid_entry_points ?? 0 }}/{{ bid.open_points }} Points
                            </span>
                            <span
                        ...
                        <div class="mb-2 h-2 w-full overflow-hidden rounded-full bg-surface-container-highest">
                            <div
                        ...
                        <div v-if="bid.ends_at" class="mb-4 text-center text-xs font-bold text-outline">
                            <span class="material-symbols-outlined mr-1 align-middle text-[14px]">schedule</span>
                        ...
                        <button
                            type="button"
                            class="w-full rounded-2xl py-4 font-bold transition-all bg-primary text-on-primary group-hover:scale-[1.02] hover:bg-on-primary-fixed active:scale-95"
                            :class="progressPct(bid) >= 100 ? 'shadow-lg shadow-primary/20' : ''"
                ...
                <div class="relative w-full max-w-md overflow-hidden rounded-3xl bg-surface-container-lowest p-6 shadow-2xl">
                    ...
                    <form @submit.prevent="submitBid">
                        <div class="mb-4">
                            ...
                            />
                            <p v-if="form.errors.points" class="mt-2 text-xs font-bold text-error">{{ form.errors.points }}</p>
                        </div>
                        ...
                        <div class="mb-6 rounded-xl border border-secondary/20 bg-secondary-container/30 p-4">
                            ...
                            <div class="flex items-center justify-between">
                                <span class="text-sm font-semibold text-on-surface-variant">Points Needed to Unlock</span>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-sm font-black text-on-surface">{{ selectedBid.bid_entry_points }}</span>
                            </div>
                        </div>
        ...
        <!-- Toast Notification -->
        <Teleport to="body">
            <div v-if="showToast" class="fixed right-6 bottom-6 z-50 flex items-center gap-3 rounded-2xl border border-surface-container bg-surface-container-highest px-6 py-4 shadow-xl">
